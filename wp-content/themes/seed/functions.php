...
function create_post_type_html5()
{
    ...
    register_post_type('html5-blank',
        array(
        'labels' => array(
...
add_action('init', 'noticia_filter');

function create_post_type_noticia()
{
    register_post_type('noticia', // Register Custom Post Type
        array(
        'labels' => array(
            'name' => __('Noticias'), // Rename these to suit
            'singular_name' => __('Noticia'),
            'add_new' => __('Nueva Noticia'),
            'add_new_item' => __('Nueva Noticia'),
            'edit' => __('Editar Noticia'),
...
function noticia_filter(){

    register_taxonomy("categoria-noticia", 
    ...
    array(
        ...
        "singular_label" => __( "categoria-noticia" ), 
        'show_ui'             => true,
        'show_in_menu'        => true,
...
add_action( 'pre_get_posts', 'post_loop_tax');

function post_loop_tax($q) {

    if( !is_admin() && $q->is_main_query() && $q->is_tax() ) { 
        
        if($q->is_tax('categoria-noticia')){
            $q->set ('post_type', array( 'post', 'noticia' ) ); $q->set ('posts_per_page', 3 );
        }
